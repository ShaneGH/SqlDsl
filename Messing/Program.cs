...
namespace SqlDsl
{
    public class Program
    {
        ...
        {
            var testClass = new JoinConditionTests();
            testClass.OneTimeSetUp();
            testClass.SetUp();
            testClass.Select1SimpleInnerJoin().Wait();
            // testClass.TearDown();
            // testClass.OneTimeTearDown();
        }
    }
    ...
    //         Console.WriteLine(rows.First().Select(cell => cell).JoinString(" "));
    //         rows
    //             .ToList()
    //             .ForEach(row => Console.WriteLine(row.Select(cell => cell).JoinString(" ")));
    //     }
